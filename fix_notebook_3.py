...
try:
    ...
    for cell in nb['cells']:
        if cell['cell_type'] == 'code':
            ...
            for line in source:
                ...
                if "x_traj, y_traj, z_traj = iterate_memristive_fhn(initial_state, params, n_steps=5000, transient=1000)" in line:
                ...
                # Fix Cell 14: Undefined variable
                elif "n_points = min(2000, len(trajectory))" in line:
                    new_source.append("n_points = min(2000, len(x_traj))\n")
                    cell_modified = True
                    modified = True
                    print("Fixed Cell 14 undefined variable.")
                else:
    ...
    if modified:
        with open(notebook_path, 'w', encoding='utf-8') as f:
            json.dump(nb, f, indent=1) # Use indent=1 to match typical notebook formatting if possible, or just default
        print(f"Successfully modified {notebook_path}")
    else:
        print("No changes were made. Code patterns not found.")
